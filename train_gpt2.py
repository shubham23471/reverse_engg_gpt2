...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_proj.NANOGPT_SCALE_INIT = 1  # like a flag for this module 
        # regularization
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        # init params 
        # (calling the apply() func from nn module that iterate all the submodule of this module)
        # and _init_weights function on them 
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Linear):
            ...
            if hasattr(module, 'NANOGPT_SCALE_INIT'):
                # we want sqrt over num of layers 
                # num of resuidual layer: 2 * self.config.n_layer
                # 2 times comes from that every single layer in our transformer 
                # actually have 2 blocks that adds to the redsiual pathwa (one is attention and other is MLP)
                    # x = x + self.attn(self.ln_1(x))
                    # x = x + self.mlp(self.ln_2(x))
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            # reshape from 3-dim to 2-dim as cross_entropy only takes 2 dim 
...
total_batch_size = 524288 # 2**19 (using nice numnber), ~0.5M, in number of tokens
B = 8 # 16 (andrej's B) # micro batch size
T = 1024 # sequence length
assert total_batch_size % (B * T * ddp_world_size) == 0, "make sure total_batch_size is divisible by B * T * ddp_world_size"
grad_accum_steps = total_batch_size // (B * T * ddp_world_size)
if master_process:
    # only print from the master process 
    print(f"total desired batch size: {total_batch_size}")
    print(f"=> calculated gradient accumulation steps: {grad_accum_steps}")


# ---------- 
# testing code to see how it works on multiple GPUs
print(f'Executed from GPU: {ddp_rank}')
...
train_loader = DataLoaderLite(B=B, T=T, power_rank=ddp_rank,
                               num_processes=ddp_world_size)

# to run on tensor cores 
torch.set_float32_matmul_precision('high')
# create model 
# because of the seed: the model wil be identical
model = GPT(GPTConfig(vocab_size=50304))
model.to(device)
model = torch.compile(model)

# wrapping the model into Pytorch's DDP container 
if ddp:
...
for step in range(max_steps):
    ...
    for micro_step in range(grad_accum_steps):
        ...
        loss_accum += loss.detach() # detach : for detaching the tensor from the graph
        # to only sync the gradient at the very last step 
        # when micro_step = grad_accum_steps 
        if ddp:
            model.require_backward_grad_sync = (micro_step == grad_accum_steps - 1)
        loss.backward()
    
    # to accumulate the loss from all GPUs and avg them
    if ddp:
    ...
    # determine and set the learning rate for this iteration, GPT=3 paper
    lr = get_lr(step)
